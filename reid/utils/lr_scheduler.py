...
class LRScheduler(object):
    ...
    def __init__(self, base_lr=0.0008, step=(30, 60), factor=0.1,
    ...
    def update(self, num_epoch):
        ...
        elif num_epoch >= self.step[0]:
            # count = sum([1 for s in self.step if s <= num_epoch])
            # self.learning_rate = self.base_lr * pow(self.factor, count)
            self.learning_rate = self.base_lr * (self.factor ** (float(num_epoch + 1 - self.step[0]) / (self.step[1] + 1 - self.step[0])))
